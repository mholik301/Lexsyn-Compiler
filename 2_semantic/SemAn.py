...
#region -----TESTING
#python prog.py < 01.in > izlaz.out
#FC izlaz.out 01.out
#endregion
...
class Varijabla:
    ...
    def isInVarijable(self, trazena):
        ...
        for var in varijable[::-1]:
            ...
            global zaUndef
            if zaUndef == True and trazena.getIme() == varijable[len(varijable)-1].getIme():
                    return 0
            #else:
            #    zaUndef = False
            if (var.getIme() == trazena.getIme() and var.getKontekst() <= trazena.getKontekst()):
                global indeks
                indeks = varijable.index(var)
...
def obilazakStabla():
    ...
    for list in stablo:
        if isinstance(list, Cestica):
            ...
            elif sifra == "OP_PRIDRUZI" and samo2 == True:
                samo = True
            elif sifra == "OP_PRIDRUZI" and samo2 == False:
                samo2 = True
            elif sifra == "IDN":
...
def dodajVar(cestica):
    ...
    if redDef == 0:
        ...
        #var vec postoji pa idemo provjeriti kontekst
        if var.getKontekst() > varijable[indeks].getKontekst() and zaUndef == True:
            varijable.append(var)
            None
        else:
            #ako samo dodjeljujemo vrijednost a ne deklariramo novu var, ne preskacemo citanje najnovije varijable
            global samo2
            samo2 = False
